...
class ZazzleProductDesigner:
    ...
    def __init__(
        ...
        if not self.affiliate_id:
            logger.error(
                "ZAZZLE_AFFILIATE_ID is not set in environment variables. Affiliate links may not work correctly."
            )
        else:
    ...
    def _get_template_config(
        ...
        """
        template_id = (
            design_instructions.template_id or ZAZZLE_PRINT_TEMPLATE.zazzle_template_id
        )
        tracking_code = (
            ZAZZLE_PRINT_TEMPLATE.zazzle_tracking_code
        )  # Always use template tracking code for now
