...
class TaskMonitor:
    ...
    def _find_stuck_tasks(self, db: Session) -> List[PipelineTask]:
        ...
        in_progress_tasks = db.query(PipelineTask).filter(
        ...
        for task in in_progress_tasks:
            ...
            if hasattr(task, 'last_heartbeat') and task.last_heartbeat:
                if now - task.last_heartbeat.replace(tzinfo=timezone.utc) > timeout_delta:
                    is_stuck = True
    ...
    async def _handle_stuck_task(self, db: Session, task: PipelineTask):
        ...
        try:
            ...
            if retry_count >= max_retries:
                logger.error(f"Task {task.id} has exceeded max retries ({max_retries}), marking as failed")
                await self._mark_task_failed(db, task, "Task stuck and exceeded max retries")
                return
            ...
            logger.info(f"Reset stuck task {task.id} to pending (retry {retry_count + 1}/{max_retries})")
    ...
    async def _restart_task(self, task: PipelineTask):
        ...
        try:
            if task.donation_id:
                ...
                logger.info(f"Restarted commission task {task.id} for donation {task.donation_id}")
            else:
                logger.warning(f"Task {task.id} is not a commission task, cannot restart automatically")
                
        except Exception as e:
            logger.error(f"Error restarting task {task.id}: {e}")
            
    async def _mark_task_failed(self, db: Session, task: PipelineTask, error_message: str):
    ...
    def get_monitoring_status(self) -> Dict[str, any]:
        ...
        return {
            ...
            "monitor_task_running": self.monitor_task is not None and not self.monitor_task.done()
        }
        ...
        try:
            ...
            for task in stuck_tasks:
                ...
                if task_info["last_heartbeat"]:
                    task_info["last_heartbeat"] = task_info["last_heartbeat"].isoformat()
