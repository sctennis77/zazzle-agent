...
class ImageProcessor:
    ...
    def __init__(
        ...
        self.signature_font_size = (
            signature_font_size or self.DEFAULT_SIGNATURE_FONT_SIZE
        )
    ...
    def stamp_image_with_logo(
        self, image: Image.Image, url: str = None, use_logo: bool = False
    ) -> Image.Image:
        """
        ...
        try:
            ...
            bordered_stamp = Image.new(
                "RGBA",
                (
                    self.stamp_size[0] + 2 * border_size,
                    self.stamp_size[1] + 2 * border_size,
                ),
                (255, 255, 255, 255),
            )  # White background
    ...
    def sign_image_with_clouvel(self, image: Image.Image) -> Image.Image:
        ...
        try:
            ...
            logger.info(
                "Image signed with Clouvel '25 signature at position (%d, %d)", x, y
            )
        ...
        for y in range(modules_count):
            for x in range(modules_count):
                if qr_matrix[y][x]:
                    ...
                    halo_img = Image.new(
                        "RGBA", (halo_size, halo_size), (255, 255, 255, 180)
                    )
                    ...
                    mask_draw.rounded_rectangle(
                        [0, 0, module_size, module_size], radius=corner_radius, fill=255
                    )

                    # Create the colored module
                    module_img = Image.new(
                        "RGBA", (module_size, module_size), color + (alpha,)
                    )
    ...
    def logo_to_qr(
        self, image: Image.Image, url: str, logo_path: str = None
    ) -> Image.Image:
        """
        ...
        """
        try:
            prepped_bg_path = os.path.join(
                os.path.dirname(__file__), "../../scripts/logo_qr_background.png"
            )
            ...
            if use_prepped_bg:
                logo_bg = (
                    Image.open(prepped_bg_path)
                    .convert("RGBA")
                    .resize((QR_SIZE, QR_SIZE), Image.LANCZOS)
                )
            else:
                logo_bg = (
                    Image.open(self.logo_path)
                    .convert("RGBA")
                    .resize((QR_SIZE, QR_SIZE), Image.LANCZOS)
                )
...
if __name__ == "__main__":
    ...
    print(
        "✅ Saved test_qr_stamp_output.png with full-size QR code and prepped background."
    )
