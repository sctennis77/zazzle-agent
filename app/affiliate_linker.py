...
class ZazzleAffiliateLinker:
    ...
    def generate_affiliate_link(self, product_data: Dict[str, str]) -> str:
        ...
        try:
            ...
            encoded_title = quote(product_data['title'])
            
            # Construct the affiliate link
            affiliate_link = (
                f"{self.base_url}?"
                f"rf={self.affiliate_id}&"
                f"product_id={product_data['product_id']}&"
                f"title={encoded_title}"
            )
            
            logger.info(f"Generated affiliate link for product: {product_data['title']}")
            return affiliate_link
            
        except Exception as e:
            logger.error(f"Error generating affiliate link: {str(e)}")
            return ""
