...
def run_interaction_job():
    ...
    try:
        ...
        products_with_actions = db.query(ProductInfo).filter(
            ProductInfo.interaction_actions.isnot(None)
        ).all()
        
        logger.info(f"Found {len(products_with_actions)} products with available actions")
        ...
        for product in products_with_actions:
            try:
                ...
                if result:
                    ...
                    logger.warning(f"No interactions processed for product {product.id}")
                    
            except Exception as e:
                logger.error(f"Error processing interactions for product {product.id}: {e}")
                continue
        
        end_time = datetime.now()
        duration = (end_time - start_time).total_seconds()
        
        logger.info(f"Interaction job completed in {duration:.2f} seconds. "
