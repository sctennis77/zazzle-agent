...
from app.zazzle_templates import (  # Import new DTOs
...
class TestZazzleProductDesigner(unittest.TestCase):
    ...
    def setUp(self):
        ...
        app_logger.setLevel(
            logging.INFO
        )  # Set level to INFO to capture info and error logs
    ...
    async def test_create_product_error(self, mock_logger):
        ...
        with patch(
            "app.zazzle_product_designer.quote", side_effect=Exception("Quote Error")
        ):
            designer = ZazzleProductDesigner()
            design_instructions = DesignInstructions(
    ...
    async def test_create_product_success_url_validation(self, mock_logger):
        ...
        designer = ZazzleProductDesigner()
        design_instructions = DesignInstructions(
            image="http://example.com/image.png", theme="test_theme"
        )
        result = await designer.create_product(design_instructions)
    ...
    async def test_create_product_missing_template(
        self, mock_get_template, mock_logger
    ):
        """Test product creation with missing template configuration."""
        designer = ZazzleProductDesigner()
        product_info = {
            "text": "test",
            "image": "http://a.com/i.png",
            "image_iid": "123",
        }
        result = await designer.create_product(product_info)
        self.assertIsNone(result)
        mock_logger.error.assert_called_with(
            "Cannot create product: Zazzle template or ZAZZLE_AFFILIATE_ID is not set."
        )
    ...
    async def test_create_product_empty_text_and_image(self, mock_logger):
        ...
        designer = ZazzleProductDesigner()
        product_info = {"text": "", "image": "", "image_iid": "", "theme": "test_theme"}
        result = await designer.create_product(product_info)
        self.assertIsNone(result)
        mock_logger.error.assert_called_once_with(
            "Missing required image URL for field: image"
        )
    ...
    async def test_create_product_missing_required_fields(self, mock_logger):
        ...
        design_instructions = DesignInstructions(
            image="", theme="test_theme", text="Test Text"
        )
        result = await designer.create_product(design_instructions)
        self.assertIsNone(result)
    ...
    async def test_create_product_invalid_url(self, mock_logger):
        ...
        designer = ZazzleProductDesigner()
        design_instructions = DesignInstructions(
            image="invalid-url", theme="test_theme", text="Test Text"
        )
        result = await designer.create_product(design_instructions)
        self.assertIsNone(result)
        mock_logger.error.assert_called_with(
            "Invalid image_url for field image: must start with http:// or https://"
        )
    ...
    async def test_create_product_with_text_color(self):
        ...
        designer = ZazzleProductDesigner()
        design_instructions = DesignInstructions(
            image="https://example.com/image.jpg", color="Red"
        )
    ...
    async def test_create_product_with_thematic_text_color(self):
        ...
        designer = ZazzleProductDesigner()
        design_instructions = DesignInstructions(image="https://example.com/image.jpg")
    ...
    async def test_create_product_with_invalid_text_color(self):
        ...
        designer = ZazzleProductDesigner()
        design_instructions = DesignInstructions(
            image="https://example.com/image.jpg", color="InvalidColor"
        )
