...
class TestSchedulerService(unittest.IsolatedAsyncioTestCase):
    ...
    def test_get_scheduler_status_with_config(self):
        ...
        self.assertTrue(
            status["last_run_at"].endswith("+00:00")
            or status["last_run_at"].endswith("Z")
        )
        self.assertTrue(
            status["next_run_at"].endswith("+00:00")
            or status["next_run_at"].endswith("Z")
        )
    ...
    def test_update_scheduler_config_existing(self):
        ...
        self.assertTrue(
            config["next_run_at"].endswith("+00:00")
            or config["next_run_at"].endswith("Z")
        )
    ...
    async def test_should_run_scheduled_commission_disabled(self):
        ...
        should_run = await self.scheduler_service.should_run_scheduled_commission(
            self.db
        )
    ...
    async def test_should_run_scheduled_commission_interval_not_passed(self):
        ...
        config = SchedulerConfig(
            enabled=True, interval_hours=24, last_run_at=recent_time
        )
        self.db.add(config)
        self.db.commit()
    ...
    async def test_should_run_scheduled_commission_interval_passed(self):
        ...
        config = SchedulerConfig(enabled=True, interval_hours=24, last_run_at=old_time)
        self.db.add(config)
        self.db.commit()
    ...
    async def test_create_scheduled_commission_success(
        self,
        mock_reddit_agent,
        mock_validator_class,
        mock_pick_subreddit,
        mock_get_subreddit_service,
    ):
        ...
        mock_validator = Mock()
        mock_validator.validate_commission = AsyncMock(
    ...
    @patch("app.services.commission_validator.CommissionValidator")
    @patch("app.agents.reddit_agent.RedditAgent")
    async def test_create_scheduled_commission_validation_fails(
        self, mock_reddit_agent, mock_validator_class, mock_pick_subreddit
    ):
