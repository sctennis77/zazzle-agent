...
def test_new_tools():
    ...
    try:
        result = agent.get_comment_context(
            "dummy_comment_id", product_id, reddit_post_id
        )
    ...
    print(
        f"{'✅' if 'get_post_context' in tools else '❌'} get_post_context tool found"
    )
    print(
        f"{'✅' if 'get_comment_context' in tools else '❌'} get_comment_context tool found"
    )

    print("\n4. Verifying enum values...")
    print(f"GET_POST_CONTEXT enum: {InteractionActionType.GET_POST_CONTEXT.value}")
    print(
        f"GET_COMMENT_CONTEXT enum: {InteractionActionType.GET_COMMENT_CONTEXT.value}"
    )
    ...
    try:
        ...
        if product_result and product_result.product_info.available_actions:
            print(
                f"✅ Available actions found: {product_result.product_info.available_actions}"
            )
        else:
        ...
        is_upvote_available = agent.is_action_available(
            product_id, InteractionActionType.UPVOTE.value
        )
    ...
    try:
        ...
        is_available = agent.is_action_available(
            product_id, InteractionActionType.GENERATE_NON_MARKETING_REPLY.value
        )
    ...
    try:
        ...
        # Check if it's available again (should not be)
        is_available = agent.is_action_available(
            product_id, InteractionActionType.GENERATE_MARKETING_REPLY.value
        )
    ...
    try:
        ...
        result = agent.marketing_reply(
            "post", "1leg4ab", test_content, "Physics", product_id, reddit_post_id
        )
        print(f"✅ marketing_reply result: {result}")

        # Check if it's available again (should not be)
        is_available = agent.is_action_available(
            product_id, InteractionActionType.MARKETING_REPLY.value
        )
    ...
    try:
        ...
        result = agent.non_marketing_reply(
            "post", "1leg4ab", test_content, "Physics", product_id, reddit_post_id
        )
        ...
        available_actions = agent.calculate_available_actions(product_id)
        remaining = available_actions.get(
            InteractionActionType.NON_MARKETING_REPLY.value, 0
        )
