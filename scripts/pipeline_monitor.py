    ...
    try:
        while True:
            ...
            try:
                ...
                runs = session.query(PipelineRun).order_by(PipelineRun.start_time.desc()).limit(10).all()
                ...
                if not runs:
                    ...
                    for run in runs:
                        ...
                        if run.end_time:
                            print(f"   ✅ Finished: {run.end_time.strftime('%Y-%m-%d %H:%M:%S')}")
                        ...
                        products = session.query(ProductInfo).filter_by(pipeline_run_id=run.id).all()
                        errors = session.query(ErrorLog).filter_by(pipeline_run_id=run.id).all()
                        ...
                        if products:
                            for product in products:
                                print(f"      • {product.theme} ({product.product_type})")
                        
                        print()
                
                # Show summary statistics
                total_runs = session.query(PipelineRun).count()
                completed_runs = session.query(PipelineRun).filter_by(status=PipelineStatus.COMPLETED.value).count()
                failed_runs = session.query(PipelineRun).filter_by(status=PipelineStatus.FAILED.value).count()
                running_runs = session.query(PipelineRun).filter_by(status=PipelineStatus.STARTED.value).count()
                
                print(f"📈 Summary: {total_runs} total runs | {completed_runs} completed | {failed_runs} failed | {running_runs} running")
