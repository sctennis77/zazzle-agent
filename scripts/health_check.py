...
def check_python_environment():
    ...
    try:
        ...
        dependencies = [
            'fastapi', 'sqlalchemy', 'pandas', 'requests', 
            'openai', 'PIL', 'uvicorn', 'alembic'
        ]
...
def check_database():
    ...
    try:
        ...
        try:
            ...
            recent_runs = session.query(PipelineRun).order_by(PipelineRun.start_time.desc()).limit(1).all()
            if recent_runs:
                last_run = recent_runs[0]
                print(f"✅ Last pipeline run: {last_run.start_time.strftime('%Y-%m-%d %H:%M:%S')} ({last_run.status})")
            else:
...
def check_api_server():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # Test API endpoint
            try:
                response = requests.get('http://localhost:8000/api/generated_products', timeout=5)
...
def check_environment_variables():
    ...
    for var in required_vars:
        value = os.getenv(var)
        if value:
            print(f"✅ {var}: Set ({value[:5]}...{value[-5:] if len(value) > 10 else ''})")
        else:
            print(f"❌ {var}: MISSING")
            all_good = False
