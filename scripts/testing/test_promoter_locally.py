...
def test_promoter_agent():
    ...
    try:
        ...
        print(
            f"   • Karma building enabled: {status.get('karma_building_enabled', 'Unknown')}"
        )
        print(
            f"   • Promotional probability: {status.get('promotional_probability', 'Unknown')}"
        )
        print(f"   • Total scanned: {status.get('total_scanned', 0)}")
        print(f"   • Total promoted: {status.get('total_promoted', 0)}")
        print(
            f"   • Karma building engagements: {status.get('karma_building_engagements', 0)}"
        )
        print(
            f"   • Unique karma subreddits: {status.get('unique_karma_subreddits', 0)}"
        )
        print(
            f"   • Available karma subreddits: {status.get('karma_subreddits_available', 0)}"
        )
        ...
        if karma_results:
            ...
            for result in karma_results[:3]:  # Show first 3 results
                ...
                print(
                    f"     - r/{subreddit}: {action} {f'(Error: {error})' if error else ''}"
                )
        else:
        ...
        for i in range(3):
            ...
            print(
                f"   • Pattern {i+1}: {pattern['name']} ({pattern['link_placement']} placement, {pattern['length_target']} length)"
            )
